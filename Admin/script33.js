...
let newmenu = document.getElementById("newinit")
...
let newarr = document.querySelectorAll('.newadd')
...
newbtn.addEventListener('click', async () => {
  ...
  if(!verifyfill){
    ...
    for(let i=0; i< newarr.length; i++){
      sendarr[i] = newarr[i].value
    }
    let newemp = await fetch('/addemp', {
      'method': 'POST',
      'headers': {
    ...
    let newresp = await newemp.json()
    if(newresp.adding == 'failed'){
      window.alert("The Request Couldn,t be fulfilled at the Moment\nPlease try Again Later")
    }
    else{
      window.alert("The New Employee's info has been added to the Database\nThey can Login Now")
    }
  }
})
...
window.addEventListener('load', async () => {
  let qtmp = JSON.parse(sessionStorage.getItem("obj"))
  let query = qtmp.self
  localStorage.setItem('allrows', JSON.stringify(qtmp))
